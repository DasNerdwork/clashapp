...
function splitRiotId(text) {
    const hashIndex = text.indexOf('#');
    let name = (hashIndex === -1 ? text : text.slice(0, hashIndex)).trim();
    let tag  = (hashIndex === -1 ? '' : text.slice(hashIndex + 1)).trim().toUpperCase();
    if (tag === '' || 'EUW'.startsWith(tag)) tag = 'EUW';
    return { name, tag };
}

function addToSearchHistory(entry) {
    let history = JSON.parse(localStorage.getItem("SearchHistory") || "[]");
    history = history.filter(e => e !== entry); // Duplikate vermeiden
    history.push(entry);
...
async function sanitize(text) {
    ...
    const { name, tag } = splitRiotId(text);
    var re = RegExp("^[" + characterSet + "\\s'-]+$"); // characterSet OHNE \u0023
    if (!name.match(re) || name.length < 3 || name.length > 16) {
        alert("ERROR: Eingabe fehlerhaft.\nName: 3-16 Zeichen, Buchstaben/Zahlen.");
        return;
    }
    document.getElementById("main-search-loading-spinner").style.opacity = 1;
    try {
        await (window.csrfTokenReady || Promise.resolve());
        const res = await fetch("/clashapp/ajax/search-resolve.php", {
            method: "POST",
            headers: {
            ...
            body: new URLSearchParams({ gameName: name, tagLine: tag })
        });
        let data = null;
        try { data = await res.json(); } catch (e) {}
        if (res.ok && data && data.redirect) {
            addToSearchHistory(name + "#" + tag);
            window.location.href = data.redirect;
        } else if (res.status === 404) {
            alert("Spieler " + name + "#" + tag + " wurde nicht gefunden.");
        } else {
            alert("Suche momentan nicht möglich, bitte später erneut versuchen.");
        }
    } finally {
        document.getElementById("main-search-loading-spinner").style.opacity = 0;
    }
}
